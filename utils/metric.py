...
from torch.nn.functional import cross_entropy, l1_loss, binary_cross_entropy_with_logits, kl_div
...
class Metric(object):
    ...
    def kl(self, p, q, type):
        if type == 'self':
            d_kl = self.kl_div(p, q) / p.size(1)
        elif type == 'torch':
            d_kl = kl_div(input=q, target=p, reduction='batchmean')
        elif type == 'entropy':
            d_kl = torch.sum(p * torch.log(p))
        return d_kl
