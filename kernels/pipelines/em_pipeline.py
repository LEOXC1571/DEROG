...
from utils import nan2zero_get_mask, pbar_setting, color_palette, eval_data_preprocess, eval_score
...
class EMPipeline:
    def __init__(self, task,
                 ...
                 ood_algorithm,
                 config):
        super(EMPipeline, self).__init__()
        self.task = task
        self.model = model
        self.loader = loader
    ...
    def train_batch(self, data, pbar):
        ...
        e_output = self.model.e_predictor(data=data, edge_weight=edge_weight, ood_algorithm=self.ood_algorithm)
        raw_e_pred = self.ood_algorithm.output_postprocess_e(e_output)
        loss_e = self.ood_algorithm.loss_calculate_e(raw_e_pred, targets, mask, node_norm, self.config)
        loss_e = self.ood_algorithm.loss_postprocess_e(loss_e, data, mask, self.config)
        self.ood_algorithm.backward_e(loss_e)

        with torch.no_grad():
            e_temp_output = self.model.e_predictor(data=data, edge_weight=edge_weight, ood_algorithm=self.ood_algorithm)
            self.ood_algorithm.output_postprocess_e_temp(e_temp_output)

        m_output = self.model.m_predictor(data=data, edge_weight=edge_weight, ood_algorithm=self.ood_algorithm)
        raw_m_pred = self.ood_algorithm.output_postprocess_m(m_output)
        loss_m = self.ood_algorithm.loss_calculate_m(raw_m_pred, targets, mask, node_norm, self.config)
        loss_m = self.ood_algorithm.loss_postprocess_m(loss_m, data, mask, self.config)
        self.ood_algorithm.backward_m(loss_m)

        return {'loss': loss_e.detach() + loss_m.detach()}
    ...
    def train(self):
        ...
        for epoch in range(self.config['train']['ctn_epoch'], self.config['train']['max_epoch']):
            ...
            print(f'#IN#\033[1;35mEpoch {epoch}:\033[0m')
            ...
            for index, data in pbar:
                ...
                # train a batch
                self.ood_algorithm.optimizer_e.zero_grad()
                self.ood_algorithm.optimizer_m.zero_grad()
            ...
            # --- scheduler step ---
            self.ood_algorithm.scheduler_e.step()
            self.ood_algorithm.scheduler_m.step()
    ...
    def evaluate(self, split):
        ...
        for data in pbar:
            ...
            if mask is None:
                return stat
            node_norm = (torch.ones((data.num_nodes,), device=self.config['device'])
                         if self.config['model']['model_level'] == 'node' else None)
            data, targets, mask, node_norm = self.ood_algorithm.input_preprocess(data, targets, mask, node_norm,
                                                                                 self.model.training,
                                                                                 self.config)
            e_output = self.model.e_predictor(data=data, edge_weight=None, ood_algorithm=self.ood_algorithm)
            raw_e_pred = self.ood_algorithm.output_postprocess_e(e_output)
            m_output = self.model.m_predictor(data=data, edge_weight=None, ood_algorithm=self.ood_algorithm)
            raw_preds = self.ood_algorithm.output_postprocess_m(m_output)
            # --------------- Loss collection ------------------
            loss = self.config['metric'].loss_func(raw_preds, targets, reduction='none') * mask
            # loss = self.config['metric'].loss_func(raw_e_pred, targets, reduction='none') * mask
            mask_all.append(mask)
            loss_all.append(loss)

            # ------------- Score data collection ------------------
            pred, target = eval_data_preprocess(data.y, raw_preds, mask, self.config['metric'].dataset_task)
        ...
        print(f'\n#IN#\033[1;34m {split.capitalize()} {self.config["metric"].score_name}: {stat["score"]:.4f}\033[0m, '
              f'\033[1;31m {split.capitalize()} Loss: {stat["loss"]:.4f}\033[0m')
    ...
    def config_model(self, mode, load_param=False):
        ...
        if mode == 'test':
            ...
            if os.path.exists(self.config['id_test_ckpt']):
                ...
                print(f'#IN#Checkpoint {id_ckpt["epoch"]}: \n-----------------------------------\n'
                      ...
                      f"\033[1;32mID Validation {self.config['metric'].score_name}: {id_ckpt['id_val_score']:.4f}\033[0m\n"
                      f'ID Validation Loss: {id_ckpt["id_val_loss"].item():.4f}\n'
                      f"\033[1;33mID Test {self.config['metric'].score_name}: {id_ckpt['id_test_score']:.4f}\033[0m\n"
                      f'ID Test Loss: {id_ckpt["id_test_loss"].item():.4f}\n'
                      f"\033[1;32mOOD Validation {self.config['metric'].score_name}: {id_ckpt['val_score']:.4f}\033[0m\n"
                      f'OOD Validation Loss: {id_ckpt["val_loss"].item():.4f}\n'
                      f'\033[1;33mOOD Test {self.config["metric"].score_name}: {id_ckpt["test_score"]:.4f}\033[0m\n'
                ...
                print(f'#IN#Checkpoint {ckpt["epoch"]}: \n-----------------------------------\n'
                      ...
                      f'\033[1;32mID Validation {self.config["metric"].score_name}: {ckpt["id_val_score"]:.4f}\033[0m\n'
                      f'ID Validation Loss: {ckpt["id_val_loss"].item():.4f}\n'
                      f'\033[1;33mID Test {self.config["metric"].score_name}: {ckpt["id_test_score"]:.4f}\033[0m\n'
                      f'ID Test Loss: {ckpt["id_test_loss"].item():.4f}\n'
                      f'\033[1;32mOOD Validation {self.config["metric"].score_name}: {ckpt["val_score"]:.4f}\033[0m\n'
                      f'OOD Validation Loss: {ckpt["val_loss"].item():.4f}\n'
                      f'\033[1;33mOOD Test {self.config["metric"].score_name}: {ckpt["test_score"]:.4f}\033[0m\n'
                ...
                print(f'#IN#Checkpoint {ckpt["epoch"]}: \n-----------------------------------\n'
                      f'Train {self.config["metric"].score_name}: {ckpt["train_score"]:.4f}\n'
                      f'Train Loss: {ckpt["train_loss"].item():.4f}\n'
                      f'\033[1;32mValidation {self.config["metric"].score_name}: {ckpt["val_score"]:.4f}\033[0m\n'
                      f'Validation Loss: {ckpt["val_loss"].item():.4f}\n'
                      f'\033[1;33mTest {self.config["metric"].score_name}: {ckpt["test_score"]:.4f}\033[0m\n'
    ...
    def save_epoch(self, epoch, train_stat, id_val_stat, id_test_stat, val_stat, test_stat, config):
        ...
        saved_file = os.path.join(config["ckpt_dir"], f'{config["time"]}_{epoch}.ckpt')
        torch.save(ckpt, saved_file)
        shutil.copy(saved_file, os.path.join(config["ckpt_dir"], f'{config["time"]}_last.ckpt'))
        ...
        if id_val_stat.get('score') and (
            ...
            shutil.copy(saved_file, config['id_test_ckpt'] )
        ...
        if config['metric'].best_stat['score'] is None or config['metric'].lower_better * val_stat[
            ...
            shutil.copy(saved_file, config['test_ckpt'])
